...
class MsgpackParser:
    ...
    def parse_symbol(self, symbol:str):
        ...
        # collect files
        update_files = glob.glob(f"{str(self.catalystBase.joinpath(symbol))}/*incremental*")
        trade_files = glob.glob(f"{str(self.catalystBase.joinpath(symbol))}/*trades*")
        snapshot_files = glob.glob(f"{str(self.catalystBase.joinpath(symbol))}/*snapshot*")
    ...
    def _parse_updates(self, update_file):
        ...
        with gzip.open(update_file) as file:
            ...
            msgpack.dump(updates, open(self.catalystBase.joinpath("l2_updates").joinpath(f"{update_file.split('.')[0]}.mpk"), "wb+"))
    ...
    def _parse_snapshots(self, snapshot_file):
        ...
        with gzip.open(snapshot_file) as file:
            ...
            msgpack.dump(snapshots, open(self.catalystBase.joinpath("ob_snapshot_50").joinpath(f"snapshot{snapshot_file.split('.')[0]}.mpk"), "wb+"))
